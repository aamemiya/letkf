...
integer,parameter::nt_train=2000
integer,parameter::nt_sync=800
...
real(r_size),parameter::vreg_in=0.0002
...
integer,parameter::nt_train_mini=1
real(r_size)::x_train(nx,nt_train_mini)
real(r_size)::x_b_train(nx,nt_train_mini)
real(r_size),parameter::vnu=0.01
...
! read(10)  x4
 if (nt_train_mini.ne.1) x_train(1:nx,2:nt_train_mini)=x_train(1:nx,1:nt_train_mini-1)
 x_train(1:nx,1)=real(x4,r_size)
 if (it.ne.1.and.mod(it-1,nt_train_mini).eq.0) call rsv_rnet_train_seq(nt_train_mini,x_b_train,x_train,vnu)
 x_b_train=x_train
 if (mod(it,100).eq.0) write(*,*) it,' / ',nt_train
end do
...
! read(30)  x4_t
if (it-nt_sync.le.100) write(*,*) sqrt(sum((x4_t-x4)**2)/real(nx))
 istat=NF_PUT_VARA_REAL(idnco,idvvo,(/1,it/),(/nx,1/),x4)
! write(90) x4(1:nx)
end do
